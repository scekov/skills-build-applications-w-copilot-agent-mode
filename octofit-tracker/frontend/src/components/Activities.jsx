...
function Activities() {
  ...
  useEffect(() => { fetchItems('activities', activitiesEndpoint).then(setActivities).catch((error) => setState({ loading: false, error: error.message })).finally(() => setState((current) => ({ ...current, loading: false }))) }, [])
